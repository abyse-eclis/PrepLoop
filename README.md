...
- 📤 **Export ข้อมูลการเรียน** — รายวัน / รายสัปดาห์ / รายเดือน / กำหนดเอง / ทั้งหมด
  เลือกได้ 5 รูปแบบ: CSV สรุปรายวัน, CSV บันทึกการเรียน, CSV รายการตามแผน, CSV ผลสอบ และ JSON ครบทุกอย่าง
  (CSV ใส่ BOM ให้ Excel อ่านภาษาไทยได้ และกัน formula injection)
...
หน้า `/progress` มีการ์ด **ส่งออกข้อมูลการเรียน** — เลือกช่วงเวลาแล้วกดดาวน์โหลด

| ช่วง | ความหมาย |
|---|---|
| รายวัน | วันนี้ (ตาม timezone ของ workspace) |
| รายสัปดาห์ | จันทร์–อาทิตย์ของสัปดาห์นี้ (ISO week) |
| รายเดือน | วันที่ 1 ถึงวันสุดท้ายของเดือนนี้ |
| กำหนดเอง | เลือกวันเริ่ม–วันสิ้นสุดเอง |
| ทั้งหมด | ตั้งแต่วันแรกที่มีข้อมูลจนถึงวันล่าสุด (server หาให้เอง) |

เบื้องหลังเป็น `GET /api/export?range=…&format=…&start=…&end=…` (ต้อง login, กรองด้วย workspace + RLS)
ช่วงเวลาถูก resolve ที่ server เสมอ ไฟล์เลยตรงกับป้ายที่เลือกเสมอแม้นาฬิกาเครื่องจะไม่ตรง timezone ของ workspace

รายการตามแผนจะถูกกรองด้วยเวอร์ชันที่ "เป็นเจ้าของ" วันนั้นจริง ๆ ช่วงที่คร่อม Recovery Plan
จึงไม่นับซ้ำสองเวอร์ชัน
